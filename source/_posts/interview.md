...
![](https://npm.onmicrosoft.cn/o-bed@1.0.3/img/interview/3.png)
{% endfolding %}
...
3. `opacity: 0`: 将透明度设置为0,占位置
4. `position: absolute`: 将元素定位到可视区域外
5. `transform: scale(0)`: 缩放元素到0
6. `z-index: -1`: 将元素层级设置为-1,使其隐藏在其他元素后面
{% endfolding %}
### css伪类
{% folding 查看答案 %}
```css
...
4. 非tabbar页面可以按照实际需求放入不同的分包内,然后进行按需下载

如何进行分包:
开发者可以通过在`app.json`文件的`subPackages`字段声明项目分包结构
```json
{
  "pages":[
    "pages/index",
...
{% folding 查看答案 %}
1. 调用wx.login()获取登录code
2. 调用wx.request()发送code到后端
3. 后端接收code,调用微信接口获取用户信息
4. 后端校验用户信息,如果合法,则生成token,返回给前端
5. 前端收到token,存储起来,后续请求携带token进行认证
...
[uniapp生命周期](https://juejin.cn/post/7104821773547339784)
